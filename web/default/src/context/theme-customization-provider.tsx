...
type ThemeCustomizationContextType = {
  ...
  setCustomHue: (customHue: number | null) => void
...
export function ThemeCustomizationProvider(props: {
  ...
  const [customHue, _setCustomHue] = useState<number | null>(() => {
    const value = getCookie(THEME_COOKIE_KEYS.customHue)
    if (!value) return DEFAULT_THEME_CUSTOMIZATION.customHue
    const parsed = Number(value)
    return Number.isFinite(parsed) ? parsed : DEFAULT_THEME_CUSTOMIZATION.customHue
  })
  ...
  useEffect(() => {
    ...
    applyThemeColors(generateThemeColors(customHue), resolvedTheme)
  }, [customHue, resolvedTheme])
  ...
  const setCustomHue = useCallback((value: number | null) => {
    if (value === null) {
      _setCustomHue(null)
      removeCookie(THEME_COOKIE_KEYS.customHue)
      removeThemeColors()
      return
    }

    _setCustomHue(value)
    setCookie(THEME_COOKIE_KEYS.customHue, String(value), COOKIE_MAX_AGE)
  }, [])
  ...
  const resetCustomization = useCallback(() => {
    ...
    setCustomHue(DEFAULT_THEME_CUSTOMIZATION.customHue)
  }, [setPreset, setRadius, setScale, setContentLayout, setCustomHue])

  const value = useMemo<ThemeCustomizationContextType>(
    () => ({
      defaults: DEFAULT_THEME_CUSTOMIZATION,
      customization: { preset, radius, scale, contentLayout, customHue },
      setPreset,
      setRadius,
      setScale,
      setContentLayout,
